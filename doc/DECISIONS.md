...
原本的作法是訂閱 Room 的 Flow，收藏狀態一改變畫面就同步更新。但會造成使用者在 Saved 頁按下取消收藏的瞬間，該篇文章立刻從清單中消失，如果是誤觸就沒有辦法復原，必須回到 Feed 頁重新找到那篇文章。改成重開 App 再回來時列表才會更新之後，取消收藏只會讓書籤圖示變成未收藏狀態，項目本身仍留在畫面上，使用者可以直接再按一次加回來。
